...
def generate_constraint_header(f, args):
    f.write("# This file has been auto generated by the fp2p tool.\n")
    f.write("# Do not modify it by hand!\n")
    f.write("# Files used for generation:\n")
    f.write("#   Tree file: {}\n".format(args.tree_file))
    f.write("#   Assignment file: {}\n".format(args.assignment_file))
    f.write("# More information on the website https://github.com/m-kru/fp2p.\n\n")


def vivado_generate_constraint_file(connection, args):
    with open(args.output_file, "w") as f:
        generate_constraint_header(f, args)
...
def quartus_generate_constraint_file(connection, args):
    with open(args.output_file, "w") as f:
        generate_constraint_header(f, args)

        for k, v in connection.items():
            pass
            f.write("set_location_assignment %s -to %s\n" % (v["fpga_pin"], k))
            if "set_instance_assignment" in v:
                for k1, v1 in v["set_instance_assignment"].items():
                    f.write(
                        "set_instance_assignment -name %s %s -to %s\n" % (k1, v1, k)
                    )
            f.write("\n")
...
def assign(tree, resolved_tree, args):
    ...
    if args.eda_tool == "vivado":
        vivado_generate_constraint_file(connection, args)
    elif args.eda_tool == "quartus":
        quartus_generate_constraint_file(connection, args)
